...
local function spawnOutsideCar(vehicle, location)
    QBCore.Functions.TriggerCallback('qb-garage:server:spawnvehicle', function(netId, properties)
        local veh = NetToVeh(netId)
        if not veh or not netId then
            print("ISSUE HERE: ", netId)
        end
        UpdateSpawnedVehicle(veh, vehicle, vehicle.h, vehicle.garage, properties)
        ApplyVehicleDamage(veh, vehicle)
    end, vehicle, location, false)
end

local function SaveVehicleCoords(veh, coords, heading)
    local plate = QBCore.Functions.GetPlate(veh)
    local bodyDamage = math.ceil(GetVehicleBodyHealth(veh))
    local engineDamage = math.ceil(GetVehicleEngineHealth(veh))

    local totalFuel = exports['cdn-fuel']:GetFuel(veh) 
    local damage = GetCarDamage(veh)
    local properties = QBCore.Functions.GetVehicleProperties(veh)

    TriggerServerEvent('nb-garage:server:UpdateVehicleCoords', totalFuel, engineDamage, bodyDamage, properties, plate, coords, heading, damage)
...
--events n0tbad 
RegisterNetEvent('nb-garages:client:trackCar', function(vehInfo, type)
    if vehInfo.state == "Out" then 
        if findVehFromPlateAndLocate(vehInfo.plate) then
            --QBCore.Functions.Notify("Vehicle Location on GPS", "success")
            TriggerEvent("nb-phone:client:trackSpawnNotification", "Tracking", "Vehicle Location on GPS")
        else
            vehInfo.x =  tonumber(string.format("%.1f",vehInfo.x))
            vehInfo.y =  tonumber(string.format("%.1f",vehInfo.y))   
            vehInfo.z =  tonumber(string.format("%.1f",vehInfo.z))
            vehInfo.h =  tonumber(string.format("%.1f",vehInfo.h))

            -- Track 
            if type == 0 then 
                --QBCore.Functions.Notify("Vehicle Location on GPS", "success")
                TriggerEvent("nb-phone:client:trackSpawnNotification", "Tracking", "Vehicle Location on GPS")
                SetNewWaypoint(vehInfo.x, vehInfo.y)
                local blip = AddBlipForCoord(vehInfo.x, vehInfo.y, vehInfo.z)
                ...
                CreateThread(function()
                    while true do
                        ...
                        if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, vehInfo.x, vehInfo.y, vehInfo.z, false) < 40 then
                            DrawText3D(vehInfo.x, vehInfo.y, vehInfo.z, "Carro Perdido")
                            DrawMarker(0, vehInfo.x, vehInfo.y, vehInfo.z-1.3, 0, 0, 0, 0, 0, 0, 2.0, 2.0, 2.0, 255, 255, 255, 50, 0, 0, 2, 0, 0, 0, 0)
                        end
                
                        if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, vehInfo.x, vehInfo.y, vehInfo.z, false) < 4 then
                            RemoveBlip(blip)
                            CanSpawnCar = true
                            TriggerEvent("nb-phone:client:trackSpawnNotification", "Tracking", "Car can be requested")
                            break
                        end
                    end
                end) 
            else 
                if not GetVehicleByPlate(vehInfo.plate) or not GetVehicleByPlate(vehInfo.fakeplate) then 
                    if CanSpawnCar then 
                        vehInfo.engine =  vehInfo.engine * 10
                        vehInfo.body =  vehInfo.body * 10
                        local location = vector4(vehInfo.x,vehInfo.y,vehInfo.z,vehInfo.h)
                        spawnOutsideCar(vehInfo, location)
                        CanSpawnCar = false
                    else
                        TriggerEvent("nb-phone:client:trackSpawnNotification", "Tracking", "Error, track again!")
                    end
                else 
                    QBCore.Functions.Notify("The is somewhere", "success")
                end
            end
        end
    else
        QBCore.Functions.Notify("Vehicle in garage: " .. vehInfo.garage, "success")
    end
end)
    ...
    while true do
      	...
      	if IsPedInAnyVehicle(ped, false) then
			if GetIsTaskActive(ped, 2) then
				--local data = QBCore.Functions.GetVehicleProperties(car)
                --local plate = QBCore.Functions.GetPlate(car)
				local coords = GetEntityCoords(car, false)
                local heading = GetEntityHeading(car)

				--TriggerServerEvent('qb-phone:server:UpdateVehicleCoords', coords, heading, data, plate)
                --Update door damages, etc
                --TriggerServerEvent("qb-vehicletuning:server:SaveVehicleProps", data)
                --TriggerServerEvent("nb-rental:server:checkRentalCarTime", plate)
                SaveVehicleCoords(car, coords, heading)
                print("Saving Car Pos")    
              	Sleep = 5000
        	end
      	end
      	Wait(Sleep)
