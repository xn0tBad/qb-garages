...
RegisterNetEvent("nb-garage:server:UpdateVehicleCoords", function(fuel, engineDamage, bodyDamage, properties, plate, coords, heading, damage)
    local originalPlate = exports['brazzers-fakeplates']:getPlateFromFakePlate(plate)
    if originalPlate then plate = originalPlate end
    MySQL.update('UPDATE player_vehicles SET state = 0, damage = ?, fuel = ?, engine = ?, body = ?, mods = ?, x = ?, y = ?, z = ?, h = ? WHERE plate = ?',{json.encode(damage), fuel, engineDamage, bodyDamage, json.encode(properties), coords.x, coords.y, coords.z, heading, plate})
end)

QBCore.Functions.CreateCallback('qb-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, warp)
    if not vehInfo.vehicle then 
        vehInfo.vehicle = string.lower(vehInfo.model)
    end
